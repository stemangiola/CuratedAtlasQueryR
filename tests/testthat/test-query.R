...
test_that("The assays argument to get_SingleCellExperiment controls the number of returned assays", {
  ...
  get_SingleCellExperiment(meta, cache_directory = LOCAL_HCA, assays = c("counts", "cpm")) |>
    assays() |>
    names() |>
    expect_setequal(c("counts", "cpm"))

  # If we request one assay, we get one assays
  get_SingleCellExperiment(meta, cache_directory = LOCAL_HCA, assays = "counts") |>
    assays() |>
    names() |>
    expect_setequal("counts")
  get_SingleCellExperiment(meta, cache_directory = LOCAL_HCA, assays = "cpm") |>
    assays() |>
    names() |>
    expect_setequal("cpm")
})
...
test_that("The features argument to get_SingleCellExperiment subsets genes", {
  ...
  sce_full <- get_SingleCellExperiment(meta, cache_directory = LOCAL_HCA) |>
    row.names() |>
    length()
  expect_gt(sce_full, 1)

  # The subset dataset should only have one gene
  sce_subset <- get_SingleCellExperiment(meta, cache_directory = LOCAL_HCA, features = "PUM1") |>
    row.names() |>
    length()
