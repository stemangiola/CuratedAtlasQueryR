...
test_that("The genes argument to get_SingleCellExperiment subsets genes", {
  ...
  sce_full = get_SingleCellExperiment(meta, repository = LOCAL_HCA_RAW) |> row.names() |> length()
  expect_gt(sce_full, 1)
  
  # The subset dataset should only have one gene
  sce_subset = get_SingleCellExperiment(meta, repository = LOCAL_HCA_RAW, genes = "PUM1") |> row.names() |> length()
...
test_that("get_seurat() returns the appropriate data in Seurat format", {
  ...
  sce = get_SingleCellExperiment(meta, repository = LOCAL_HCA_RAW, genes = "PUM1")
  seurat = get_seurat(meta, repository = LOCAL_HCA_RAW, genes = "PUM1")
