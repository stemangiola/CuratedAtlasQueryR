...
Saving as `.rds` has the advantage that rewrites on disk a monolithic `HDF5` and so displaying and manipulating large `SingleCellExperiment` objects, which merge a lot of HDF5 from your `get_single_cell_experiment`, is going to be fast.
...
```{r}
single_cell_counts |> HDF5Array::saveHDF5SummarizedExperiment("single_cell_counts")
```
